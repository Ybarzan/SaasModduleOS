  ...
  return (
    <>
      <button
        ...
        className="md:hidden fixed top-3 right-3 z-40 p-2.5 rounded-none bg-surface/95 backdrop-blur-lg border border-line shadow-md text-ink-soft"
      >
        <Menu size={20} />
      </button>
          ...
          <div className="h-16 flex items-center justify-between px-4 border-b border-line shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-2 rounded-none text-ink-soft hover:bg-surface-2"
              >
                <X size={22} />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1 pb-safe">
            {STANDALONE_ITEMS.filter((item) => isItemVisible(item, user?.role)).map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.to}
                  onClick={() => setOpen(false)}
                  className={`flex items-center gap-3 px-3 py-3 rounded-none text-sm font-semibold ${
                    item.highlight
                ...
                return (
                  <div key={group.id}>
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-3 rounded-none text-sm font-medium text-ink hover:bg-surface-2"
                    >
                        ...
                        {group.items.map((item) => {
                          ...
                          if (locked) {
                            return (
                              <Link
                                ...
                                className="flex items-center gap-3 px-3 py-2.5 rounded-none text-sm text-ink-soft/50"
                              >
                          ...
                          return (
                            <Link
                              ...
                              to={item.to}
                              onClick={() => setOpen(false)}
                              className={`flex items-center gap-3 px-3 py-2.5 rounded-none text-sm ${
            ...
            <div className="border-t border-line pt-3 mt-3">
              ...
              <button
                ...
                className="w-full text-left px-3 py-3 rounded-none text-sm font-medium text-accent hover:bg-accent-soft flex items-center gap-2"
              >
                <LogOut size={16} />
                Déconnexion
              </button>
