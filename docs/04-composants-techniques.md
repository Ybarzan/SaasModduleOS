...
**Partie `condition` faite, 2026-08-23** : `RuleConditionNode` (interprète maison, exactement l'approche recommandée ci-dessus, pas de DSL) + `RuleConditionEvaluator`, branché sur `NotificationRule.conditionJson`. AND/OR imbriqués, 8 opérateurs (EQ/NEQ/CONTAINS/IN/GT/GTE/LT/LTE). Coexiste avec l'ancien filtrage plat (`filterStatus`/`filterCarrierId`/`filterDataSource`) sans le casser — une règle utilise l'un ou l'autre, jamais les deux. **Restent à faire** : `actions candidates` typées (aujourd'hui `NotificationRule` ne sait que notifier, pas agir) et `contraintes de gouvernance` — les deux prérequis de l'exécuteur d'actions ci-dessous. Pas de constructeur de règles visuel (AND/OR imbriqués) côté frontend pour l'instant.
