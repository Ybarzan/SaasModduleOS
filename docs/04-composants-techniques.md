...
**Flux d'approbation fait, 2026-08-23** : `OrchestrationSuggestionService` + `OrchestrationSuggestionController` (`/v1/orchestration-suggestions`, GET liste/détail, POST `{id}/approve`/`{id}/reject`, rôle OWNER/ADMIN/MANAGER requis, plan PRO minimum). Machine à états simple (`PENDING_APPROVAL` → `APPROVED`/`REJECTED`, transition impossible depuis un autre statut) avec horodatage + utilisateur + note de décision. **Ce flux ne fait qu'approuver ou rejeter — il n'exécute toujours rien de réel.** L'exécuteur proprement dit (consommer une suggestion `APPROVED`, appeler `ErpProvider.exportOrders`, gérer l'échec partiel comme décrit ci-dessus, passer la suggestion à `EXECUTED`/`FAILED`) reste le seul morceau non construit de la Phase 3 J6-J8.
