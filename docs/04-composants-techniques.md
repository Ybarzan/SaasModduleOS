...
**Constructeur visuel de conditions faite, 2026-08-24** : `ConditionTreeEditor.tsx` + `lib/conditionTree.ts` (arbre AND/OR imbriqué, 8 opérateurs, ajout/suppression de conditions et de groupes récursivement) branché dans `NotificationRules.tsx` via un bascule "Filtres simples" / "Conditions avancées (ET/OU)" — mutuellement exclusifs, exactement comme le backend l'exige (`conditionJson` remplace entièrement `filterStatus`/`filterCarrierId`/`filterDataSource`, jamais les deux ensemble). Les champs disponibles par condition dépendent du type d'événement choisi (`EVENT_CONDITION_FIELDS`), reflétant exactement les clés que `NotificationService` place dans `templateData` pour chaque événement — un champ hors de cette liste n'aurait aucune donnée à comparer côté backend. A révélé au passage que `DPS_ALERT` (event type existant côté backend depuis `NotificationService.onDpsAlert`) n'était même pas sélectionnable dans le formulaire — ajouté à `EVENT_TYPES`. La carte de règle affiche désormais la condition composée sous forme de phrase lisible (`describeCondition`) au lieu du résumé de filtres plats quand `conditionJson` est présent. **Ceci ferme le dernier morceau explicitement laissé API-only dans docs/03 — toute la chaîne (création de règle, y compris conditions composées, jusqu'à l'exécution) est maintenant pilotable depuis l'UI.**

**Preuve de bout en bout faite, 2026-08-24** : `OrchestrationSuggestionE2eTest` (package `e2e`, `TestRestTemplate` + vrai serveur embarqué + vraie base H2 — pas de mocks). Jusqu'ici la Phase 3 était testée maillon par maillon (chaque service isolé avec ses dépendances mockées) mais jamais assemblée de bout en bout à travers de vrais appels HTTP. 4 scénarios : création de règle (API réelle) → vrai changement de statut d'expédition → vrai traitement de l'outbox (`EventOutboxProcessor.processPendingEvents()` appelé directement, pas d'attente du `@Scheduled` de 30s) → vraie suggestion PENDING_APPROVAL relisible par API ; approbation sans config ERP active → FAILED ; approbation avec coût au-delà du budget de la règle → FAILED, gouvernance bloquante avant tout appel ERP ; rejet → REJECTED, jamais d'exécution tentée. Aucun `ErpProvider` mocké : les scénarios choisis échouent avant d'atteindre le connecteur, donc ils prouvent le câblage réel sans dépendre d'un appel externe — le succès de l'appel `exportOrders` lui-même reste couvert en isolation par `OrchestrationExecutorTest`.

**Interface de création côté humain faite, 2026-08-24** : même problème que ci-dessus mais côté amont — `NotificationRules.tsx` n'exposait aucun champ `actionType`/`maxBudgetAmount`/`allowedCarrierIds` dans son formulaire, alors que ce sont ces champs (V65) qui font qu'une règle produit ou non une `OrchestrationSuggestion`. Sans cet ajout, la page de décision ci-dessus n'aurait jamais rien eu à afficher pour une règle créée depuis l'UI. Ajout d'un sélecteur d'action (une seule option pour l'instant : `SUGGEST_ERP_ORDER_ADJUSTMENT`) qui révèle budget max + sélection multiple de transporteurs autorisés quand une action est choisie, avec un rappel explicite du principe R1 (docs/05) directement dans le formulaire. `requiresApproval` **volontairement pas exposé** : ce champ est persisté/validé côté backend mais n'est lu nulle part dans le code d'exécution (`OrchestrationSuggestionService`/`OrchestrationExecutor` imposent toujours `PENDING_APPROVAL` puis décision humaine, quel que soit sa valeur) — exposer un toggle qui n'a aucun effet réel aurait été trompeur. Toujours pas de constructeur visuel pour `conditionJson` (arbre AND/OR) : seuls les filtres plats existants restent éditables depuis l'UI pour la condition, l'action/gouvernance est le seul ajout de cette passe.
