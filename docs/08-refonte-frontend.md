...
**fleet-hub — palette/police/coins faits, 2026-08-23** (commit `84f14df`) : fleet-hub avait déjà son propre système de tokens CSS (`--bg`/`--primary`/`--font`/`--radius`), retonné vers la palette Praxio partagée plutôt que reconstruit. JetBrains Mono remplace Orbitron/Share Tech Mono/Inter. Plusieurs dégradés/couleurs codés en dur en bleu (logo, titre login, fond login) trouvés et corrigés en vérifiant visuellement — les tokens seuls ne suffisaient pas. Reste : les palettes de graphiques (`DriverDetail.jsx`/`TruckDetail.jsx`/`MapPage.jsx`), quelques teintes de badges secondaires, et le préfixe `::` sur `.page-header h2` (équivalent du `<h1>` de compliance-engine, un seul point de levier pour les ~21 pages).
