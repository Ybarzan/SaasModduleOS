...
**Slices 1-2 faites, 2026-08-24** (commits `216dd0b`, `3c67832`, `901dfd9`) : en l'absence d'instrumentation d'usage réelle, priorisation par ordre de `NAV_GROUPS` dans `src/config/navigation.ts` — `Import-Export` et `Transport Multimodal` sont explicitement commentés dans le code comme "le bundle Starter PME", donc traités en premier. 26 pages migrées (rounded-none, préfixe `::`, coins HUD sur l'élément vraiment saillant uniquement — jamais balayé sur une grille de cartes de poids égal) : LandedCostCalculator, VolumetricWeight, CustomsDutyCalculator, CustomsDashboard, HsClassification, TradeAgreements, Quotes, Logistics, Carriers, CarrierBookings, RateComparison, Co2Calculator, FlightRadar, CargoInsurance, RouteOptimizer, ShippingRates, EoriSettings, DeclarationsPage, DeniedPartyScreening, QualityDashboard, ApprovalWorkflows, GroupagePage, TaricData, FrenchFiscal, Eur1Certificates, Compliance. **Ferme entièrement les deux groupes de nav les plus prioritaires.** Prochain groupe par ordre de priorité : `Documents & Automatisation` (2 pages restantes hors celles déjà neuves : DocumentsPage, EmailIntake), puis `Entrepôt & Stock` (Enterprise uniquement, usage plus rare).
