...
**Reste à faire** (hors scope de cette passe, à traiter si un vrai besoin se confirme) : sélecteur de camion dans le formulaire de création/édition d'expédition (`Shipments.tsx`) pour renseigner `fleetHubTruckRegistration` depuis l'UI plutôt que par API directe ; réplanification de tournée déclenchée par l'exécuteur d'actions (Couche 3, cas d'usage 3 — explicitement différé après validation du premier cas d'usage sur un pilote réel, cf. docs/03).
