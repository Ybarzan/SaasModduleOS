...
**Reste à faire** (hors scope de cette passe, à traiter si un vrai besoin se confirme) : UI frontend pour configurer fleet-hub et assigner un camion à une expédition (aujourd'hui API-only, même limitation que les autres intégrations tierces avant leur UI) ; réplanification de tournée déclenchée par l'exécuteur d'actions (Couche 3, cas d'usage 3 — explicitement différé après validation du premier cas d'usage sur un pilote réel, cf. docs/03).
