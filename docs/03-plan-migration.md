...
| J3-J5 | Moteur de règles structuré (condition composée → action candidate → contrainte de gouvernance) | Schéma de règle versionné, moteur d'évaluation testé sur 20+ scénarios de règles représentatifs — **condition composée faite 2026-08-23** (`RuleConditionNode`/`RuleConditionEvaluator`, AND/OR/8 opérateurs, `NotificationRule.conditionJson` V64, backend uniquement, pas de constructeur visuel) ; reste : action candidate et contrainte de gouvernance (budget, transporteurs autorisés) |
