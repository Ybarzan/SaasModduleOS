...
| J3-J5 | Moteur de règles structuré (condition composée → action candidate → contrainte de gouvernance) | Schéma de règle versionné, moteur d'évaluation testé sur 20+ scénarios de règles représentatifs — **fait 2026-08-23** : condition composée (`RuleConditionNode`/`RuleConditionEvaluator`, AND/OR/8 opérateurs, `NotificationRule.conditionJson` V64) + action candidate typée (`actionType`, validé, crée une `OrchestrationSuggestion` jamais exécutée) + gouvernance (`requiresApproval`/`maxBudgetAmount`/`allowedCarrierIds`, V65 — persistés/validés, pas encore appliqués par un exécuteur qui n'existe pas). Backend uniquement, pas de constructeur visuel |
