...
| Jalon | Livrable | Critère de sortie |
|---|---|---|
| J1-J2 | Bus d'événements (pattern outbox PostgreSQL + `ApplicationEventPublisher`) | Les événements ETA dégradé / statut changé transitent par le bus, `EventPublisher` actuel migré dessus sans régression notification — **`SHIPMENT_STATUS_CHANGE` fait 2026-08-23** (table `event_outbox` V63, `EventOutboxProcessor` planifié avec retry/abandon après 5 tentatives) ; **`SHIPMENT_CREATED` migré 2026-08-24**, même pattern (`ShipmentCreatedPayload`, écrit dans la même transaction que `ShipmentService.createShipment`). `providerStatusChanged`/`quoteReceived` **volontairement laissés sur l'ancien chemin `@Async`** : audit du code (2026-08-24) confirme qu'ils n'ont aucun appelant réel aujourd'hui (`NotificationService.onProviderDown/onProviderRecovered/onQuoteReceived` existent mais rien ne les déclenche) — migrer de la plomberie autour de méthodes mortes n'aurait rien apporté ; à faire quand un vrai déclencheur (ex. health-check fournisseur) sera branché. **J1-J2 clos pour tout ce qui a un déclencheur réel aujourd'hui.** |
| J3-J5 | Moteur de règles structuré (condition composée → action candidate → contrainte de gouvernance) | Schéma de règle versionné, moteur d'évaluation testé sur 20+ scénarios de règles représentatifs — **fait 2026-08-23** : condition composée (`RuleConditionNode`/`RuleConditionEvaluator`, AND/OR/8 opérateurs, `NotificationRule.conditionJson` V64) + action candidate typée (`actionType`, validé, crée une `OrchestrationSuggestion` jamais exécutée) + gouvernance (`requiresApproval`/`maxBudgetAmount`/`allowedCarrierIds`, V65). **Constructeur visuel fait 2026-08-24** (`ConditionTreeEditor.tsx`, arbre ET/OU imbriqué dans `NotificationRules.tsx`) — la chaîne complète (créer une règle avec condition composée + action + gouvernance, jusqu'à l'approbation et l'exécution) est pilotable depuis l'UI, plus aucune étape API-only |
| J6-J8 | Exécuteur d'actions coordonnées — **1 seul cas d'usage complet en premier** (recommandé : ajustement commande fournisseur via `ErpProvider.exportOrders` sur ETA dégradé confirmé) | Un scénario de bout en bout démontrable : ETA se dégrade → règle se déclenche → commande ajustée dans Odoo/SAP, journal d'audit complet — **fait 2026-08-24** : `OrchestrationExecutor` (V66) déclenché automatiquement par `OrchestrationSuggestionService.approve()` (l'approbation humaine EST le déclencheur d'exécution) — charge l'expédition, vérifie la gouvernance (`allowedCarrierIds`/`maxBudgetAmount`, désormais réellement bloquante et non plus seulement déclarative), appelle `ErpProvider.exportOrders(config, List.of(shipment))`, fait passer la suggestion à `EXECUTED`/`FAILED` avec `executionResult` journalisé séparément de la note de décision humaine. **Phase 3 J1-J8 entièrement close.** |
